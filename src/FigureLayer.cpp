...
{
	auto it = std::find_if(operators.begin(), operators.end(), [id](std::shared_ptr<Operator> &op)
						   { return op->getId() == id; });
	if (it != operators.end())
	{
		operators.erase(it);
...
{
	auto it = std::find_if(enemies.begin(), enemies.end(), [id](std::shared_ptr<Enemy> &en)
						   { return en->getId() == id; });
	if (it != enemies.end())
	{
		enemies.erase(it);
